...
<body>
	...
	<div id="hiddenDatesCoherence" hidden>
		<spring:message code="lbl.datesCoherence" />
	</div>
			...
			<div class="row">
				<div class="col-xs-8 col-xs-offset-2 box">
					<div class="label label-default pull-right">ID :
						${computer.id}</div>
					<h1>
						<spring:message code="lbl.editComputer" />
					</h1>

					<form:form id="computerForm" action="editComputer" method="POST"
						...
						<fieldset>
							<div class="form-group">
								...
								<form:input type="text" class="form-control" id="computerName"
									value="${computer.name}" path="name" required="required" />
								<form:errors path="name" class="error"></form:errors>
							</div>
							<div class="form-group">
								<form:label path="introducedDate">
									<spring:message code="lbl.introduced" />
								</form:label>
							...
							<div class="form-group">
								<form:label path="discontinuedDate">
									<spring:message code="lbl.discontinued" />
								</form:label>
									...
									<c:forEach items="${manufacturers}" var="manufacturer">
										<c:choose>
											<c:when test="${manufacturer.id == computer.manufacturerId}">
												<form:option class="companyId-option" value="${count}"
													selected="selected">${manufacturer.name}</form:option>
											</c:when>
											<c:otherwise>
						...
						<div class="actions pull-right">
							<input id="editButton" type="submit"
								value="<spring:message code="lbl.edit"/>"
